...
SpeciesBar <- ggplot(VacRich_Count,
  ...
  labs(y = "Insect Species Count", x = "Plant Species", title = "Species Count in Native and Non-Native Plant Species")

NvNN <- ggplot(VacRich_Count,
       aes(x = Naturalization, y = Species_Count, fill = Species, color = Starfish)) + 
  geom_bar(stat = 'identity')

VacRich_Count |>
  group_by(Naturalization) |>
  count()


#SpeciesBar + scale_fill_manual(values = pnw_palette("Starfish", 6, type="discrete"))
```
